...
years = driver_standings_data[['Year']].copy().Year.unique()
...
team_bar_data = team_standings_data[team_standings_data['Year'] == to_year]

fig_team = px.bar(team_bar_data,
                    x='Team Name',
                    y='Points',
                    title='F1 Team Standings Over the Years')

bar_data = driver_standings_data[driver_standings_data['Year'] == to_year]
...
app.layout = html.Div(children=[
    ...
    #html.Hr(),
        ...
        html.Div(children=[
            ...
            dcc.Slider(years.min(), years.max(), 1, marks={years.min(): str(years.min()), years.max(): str(years.max())}, value=years.max(), tooltip={"placement": "bottom", "always_visible": True}, id="team_slider" )
...
def update_driver_year_fig(value):
    ...
    bar_data = team_standings_data[team_standings_data['Year'] == year]


    fig = px.bar(bar_data,
                x='Team Name',
                y='Points',
                title='F1 Team Points for a year',
                )
...
def update_driver_year_fig(value):
    ...
    fig = px.bar(bar_data,
                x='Full Name',
                y='Points',
                title='F1 Driver Points for a year',
                )
